...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  if (!response.ok) {
    ...
    if (ENV.openRouterApiKey) {
      try {
        return await invokeOpenRouterFallback(payload);
      } catch (fallbackError) {
        throw new Error(
          `LLM invoke failed: ${response.status} ${response.statusText} – ${errorText}; ${fallbackError instanceof Error ? fallbackError.message : "OpenRouter fallback failed"}`
        );
      }
    }
    throw new Error(
